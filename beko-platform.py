...
if selected != "لا ملفات":
    data = conn.execute(
        "SELECT content FROM files WHERE name=?", (selected,)
    ).fetchone()
...
if st.sidebar.button("💾 حفظ", type="primary"):
    conn.execute(
        "INSERT OR REPLACE INTO files VALUES ((SELECT COALESCE(MAX(id)+1,1) FROM files),?,?)",
...
if prompt:
        ...
        with st.spinner("⏳ يفكر..."):
            resp = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Belkacem Meta Ads Manager DZD COD. عربي.",
                    }
                ]
    ...
    conn.execute(
        "INSERT INTO chats (message, response) VALUES (?, ?)", (prompt, resp_text)
    )
    conn.commit()
...
with st.expander("🛠️ بناء Skill"):
    ...
    if st.button("🚀 أنشئ", type="primary"):
        resp = client.chat.completions.create(
            model="llama-3.3-70b-versatile",
            messages=[
                {"role": "system", "content": "أنشئ SKILL.md كامل."},
                {"role": "user", "content": skill_goal},
            ],
        )
