...
class SelfHealingAgent:
    ...
    def fix_missing_imports(self):
        """تثبيت تلقائي آمن"""
        critical_packages = [
            "flask", "flask-jwt-extended", "streamlit", 
            "python-telegram-bot", "groq"
        ]
    ...
    def fix_goal_file(self):
        ...
        if goal_file.exists():
            ...
            if "DZD" not in content and "act_" not in content:
                ...
                goal_file.write_text("Scale Meta Campaign DZD 5000 act_4330803053874384")
    
    def log_fix(self, error_type: str, details: str):
        """حفظ الإصلاح في DB"""
        conn = sqlite3.connect(config.DB_PATH)
        cursor = conn.cursor()
        cursor.execute("INSERT INTO fixes (error_type, package) VALUES (?, ?)", 
                      (error_type, details))
        conn.commit()
        conn.close()
    ...
    def save_fix_log(self):
        ...
        Path("beko_fix_log.json").write_text(json.dumps(log, ensure_ascii=False, indent=2))
        ...
        for attempt in range(max_retries):
            try:
                ...
                result = subprocess.run(
                    ...
                    capture_output=True, text=True, timeout=120,  # 2 دقائق
