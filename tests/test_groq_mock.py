...
def test_beko_self_agent_grok_call(monkeypatch, tmp_path):
    ...
    mock_client.chat = SimpleNamespace(completions=SimpleNamespace(create=lambda **kwargs: resp))
...
def test_beko_v5_grok_heal(monkeypatch):
    ...
    class Groq:
        def __init__(self, api_key):
            self.chat = SimpleNamespace(completions=SimpleNamespace(create=lambda **kwargs: resp))
